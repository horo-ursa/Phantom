...
namespace ptvk
{
        ...
        VkImageUsageFlags imageUsage) 
    {
        ...
        VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(m_physicalDevice, m_surface, &formatCount, surfFormats.data()));
    ...
    VkExtent2D SwapChain::update(int width, int height, bool vsync)
    {
        ...
        VK_ASSERT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_physicalDevice, m_surface, &surfCapabilities));
        ...
        // test against valid size, typically hit when windows are minimized, the app must prevent triggering this code accordingly
        ...
        */
        ...
        VK_ASSERT(vkCreateSwapchainKHR(m_device, &swapchain, nullptr, &m_swapchain));
        ...
        VK_ASSERT(vkGetSwapchainImagesKHR(m_device, m_swapchain, &m_imageCount, nullptr));
        m_entries.resize(m_imageCount);
        m_barriers.resize(m_imageCount);

        std::vector<VkImage> images(m_imageCount);

        VK_ASSERT(vkGetSwapchainImagesKHR(m_device, m_swapchain, &m_imageCount, images.data()));
        //
        // Image views
        //
        for (uint32_t i = 0; i < m_imageCount; i++)
        {
            ...
            VK_ASSERT(vkCreateImageView(m_device, &viewCreateInfo, nullptr, &entry.imageView));

            // semaphore
            VkSemaphoreCreateInfo semCreateInfo = { VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };

            VK_ASSERT(vkCreateSemaphore(m_device, &semCreateInfo, nullptr, &entry.readSemaphore));
            VK_ASSERT(vkCreateSemaphore(m_device, &semCreateInfo, nullptr, &entry.writtenSemaphore));
