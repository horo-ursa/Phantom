...
namespace pt {
	...
	namespace
	{
		...
		bool enable_all_extensions(
			const std::vector<const char*>             required_ext_names,
			const std::vector<vk::ExtensionProperties>& available_exts,
			std::vector<const char*>& enabled_extensions)
		{
		...
		uint32_t                                      api_version)
	{
		...
		vk::ApplicationInfo app_info(application_name.c_str(), 0, "Phantom Engine", 0, api_version);
...
#	endif
	...
	HPPInstance::HPPInstance(vk::Instance instance) : handle{instance}
	{
		if (handle)
		{
			query_gpus();
	...
	HPPPhysicalDevice& HPPInstance::get_first_gpu()
	{
	...
	void HPPInstance::query_gpus()
	{
		...
		{
			gpus.push_back(std::make_unique<HPPPhysicalDevice>(*this, physical_device));
		}
	}
